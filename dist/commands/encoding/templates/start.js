...
export default class EncodingTemplateStart extends BaseCommand {
    static description = 'Start an encoding from a YAML template file or a stored template ID';
    static args = {
    ...
    async run() {
        ...
        let result;
        if (flags.id) {
            result = await (await this.getApi()).encoding.templates.start({ id: flags.id });
        }
        else {
            const content = readFileSync(args.file, 'utf-8');
            result = await (await this.getApi()).encoding.templates.start(content);
        }
