...
export default class EncodingCodecList extends BaseCommand {
    static description = 'List codec configurations';
    static flags = {
    ...
    async run() {
        ...
        if (codec === 'h264') {
            ...
            const allItems = (await api.encoding.configurations.list(listFn)).items ?? [];
            if (type === 'video') {
                const videoCodecs = new Set(['H264', 'H265', 'AV1', 'VP9', 'VP8', 'H262', 'MJPEG']);
                items = allItems.filter((item) => videoCodecs.has(String(item.type ?? '').toUpperCase()));
            }
            else if (type === 'audio') {
                const audioCodecs = new Set(['AAC', 'OPUS', 'VORBIS', 'AC3', 'EAC3', 'MP2', 'MP3', 'DTS', 'DTS_PASSTHROUGH', 'DTS_X', 'DTSE']);
                items = allItems.filter((item) => audioCodecs.has(String(item.type ?? '').toUpperCase()));
            }
            else {
                items = allItems;
            }
