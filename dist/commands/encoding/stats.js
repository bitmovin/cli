...
export default class EncodingStats extends BaseCommand {
    static description = 'Show encoding statistics';
    static flags = {
    ...
    async run() {
        ...
        if (flags.from && flags.to) {
            const result = await (await this.getApi()).encoding.statistics.daily.listByDateRange(new Date(flags.from), new Date(flags.to));
