...
export default class EncodingManifestDelete extends BaseCommand {
  static override description = 'Delete a manifest';
  ...
  async run(): Promise<void> {
    ...
    if (flags.type === 'dash') await (await this.getApi()).encoding.manifests.dash.delete(args.id);
    else if (flags.type === 'hls') await (await this.getApi()).encoding.manifests.hls.delete(args.id);
    else await (await this.getApi()).encoding.manifests.smooth.delete(args.id);
