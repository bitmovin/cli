...
import {ORGANIZATION_COLUMNS, listOrganizations, toOrganizationRows} from '../../../lib/organizations.js';
...
export default class AccountOrganizationsList extends BaseCommand {
  ...
  async run(): Promise<void> {
    ...
    if (flags.parent) {
      ...
      selected = rows.filter((row) => (flags.type === 'sub' ? row.parentId !== null : row.parentId === null));
    }
